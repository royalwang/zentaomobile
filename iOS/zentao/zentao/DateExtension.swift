...
extension NSDate: Comparable, Equatable {
    ...
    func toString(formatStr format: String/* = "yyyy-MM-dd HH:mm:ss"*/) -> String{
        var formatter = NSDateFormatter()
        formatter.dateFormat = format
...
extension NSDate {
    ...
    private class func componentFlags() -> NSCalendarUnit {
        var flags: NSCalendarUnit = .YearCalendarUnit | .MonthCalendarUnit | .DayCalendarUnit | .WeekCalendarUnit
        flags |= .HourCalendarUnit | .MinuteCalendarUnit | .SecondCalendarUnit | .WeekdayCalendarUnit
        flags |= .WeekdayOrdinalCalendarUnit | .CalendarUnitWeekOfYear
        return flags
    }
